...
{
    int i,j;
    double fitness, diff, feval, sum1,sum2,maxFun,sum;


   //  fitness = euclidian_distance(individual->obj,reference_point,number_objective);
    maxFun = -1.0e+30;
    sum = 0.0;
    for (i = 0; i < number_objective; i++)
    {
        diff = fabs (individual->obj[i] - reference_point[i]);
        sum += diff;
        if (lambda[i] < EPS)
            feval = diff / 0.00001;
        else
...
double fitnessFunction (individual_real *individual, double *lambda)
{
    ...
    {
     double temp_sum=0.0,asf_max=-INF,temp1;
        for (i = 0; i < number_objective; i++)
        {
            temp1=lambda[i]*(individual->obj[i]-ideal_point[i]);
            temp_sum += temp1;
            if(temp1 > asf_max)
                asf_max = temp1;
        }
